...
class NeuralSrp(nn.Module):
        ...
        for conv_cnt in range(n_conv_layers):
            self.conv_block_list.append(
                ConditionalConvBlock(
                    ...
                    pool_size=(1, params["f_pool_size"][conv_cnt]),
    ...
    def _pairwise_forward(self, x, metadata=None, hidden_state=None):
        """input:
    ...
    def forward(self, x, metadata, lengths, hidden_state = None):
        ...
        if len(x_shape) != 5:
            ...
            x_shape = x.shape
        # batch_size, n_frames, n_channels, n_mic_pairs, n_features = x.shape
        '''(batch_size, time_steps, channel, n_mic_pairs, freq_bins) to (batch_size, n_mic_pairs, time_steps, freq_bins, channel)''' 
        ...
        x, hidden_state = self._pairwise_forward(
            x,
            metadata=metadata,
