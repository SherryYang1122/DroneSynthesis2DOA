...
# This code generates emitted signals from drone body according to drone type,
# flying states, weather conditions and other parameters.
...
# 5 types of drones mentioned in this paper, M2, I2, P4, F-4, S-9; more details
# in Table 1 of this paper, including weight and size;
...
# Model parameters a and b [s/m] to estimate the normalised standard deviation
# of the rotational speed variation in dependency of the wind speed
...
EQ_FREQ = np.array([0, 100, 125, 160, 200, 250, 315, 400, 500, 630, 800, 1000,
                    1250, 1600, 2000, 2500, 3150, 4000, 5000, 6300, 8000,
                    10000, 12500, 16000])
# Equalizer slope parameter S [dB/r/min] in equation for the different drone models
EQ_SLOPE = {
    'M2': np.array([1.5E-3, 1.5E-3, 2.9E-3, -1.3E-4, 4.4E-3, 3.8E-3, 4.6E-3, 4.4E-3,
                    4.3E-3, 4.0E-3, 3.6E-3, 3.7E-3, 2.7E-3, 3.8E-3, 3.8E-3, 3.6E-3,
                    3.5E-3, 3.5E-3, 3.3E-3, 3.3E-3, 3.3E-3, 2.3E-3, 7.5E-4, 1.8E-4]),
    'I2': np.array([6.6E-3, 6.6E-3, 5.4E-3, 9.8E-3, 1.1E-2, 8.7E-3, 9.3E-3, 1.1E-2,
                    8.1E-3, 8.5E-3, 8.8E-3, 7.7E-3, 7.8E-3, 8.0E-3, 8.0E-3, 8.0E-3,
                    8.1E-3, 8.4E-3, 8.4E-3, 9.0E-3, 8.3E-3, 7.9E-3, 6.2E-3, 4.9E-3]),
    'F-4': np.array([2.3E-4, 2.3E-4, 3.6E-2, 0, 0, 3.3E-2, 1.7E-2, 1.2E-2, 4.1E-2,
                     1.8E-2, 4.5E-2, 2.3E-2, 2.9E-2, 2.6E-2, 2.3E-2, 1.8E-2, 2.2E-2,
                     1.8E-2, 2.0E-2, 2.0E-2, 1.4E-2, 1.7E-2, 1.9E-2, 1.9E-2]),  # 2.3E-4 is weird
    'S-9': np.array([5.8E-3, 5.8E-3, 1.4E-3, 4.1E-3, 7.4E-3, 6.3E-3, 1.0E-2, 7.0E-3,
                     8.2E-3, 6.0E-3, 1.4E-3, 4.1E-3, 3.0E-3, 5.2E-4, 2.4E-3, 3.8E-3,
                     3.6E-3, 4.3E-3, 4.0E-3, 3.8E-3, 4.3E-3, 2.8E-3, 7.0E-4, 2.9E-3])
}
...
def getFlightPath(starting_point, flight_state, drone_type="M2", forward_dir=[], wind_speed=2.5,
                  down_wind=True, climb_speed_max=4, sink_speed_max=3, speed_min=2):
...
def getOneRotorSignal(rpm_matrix, rpm_std, freq_mod, sampling_rate, drone_loudness,
                      signaltype='rect', change_time=0.1, s_shape_coef=5, pulse_len=1, random_number=0):
    ...
    for kk, end_time_s in enumerate(time_vec):
        ...
        act_pulse_freq[:int(sampling_rate * change_time)] = generate_s_shape_curve(
            freq0, freq1, sampling_rate, change_time, s_shape_coef)
    ...
    while start_time_s < len(base_freq):
        ...
        real_freq = (np.random.normal(base_freq[start_time_s], base_freq[start_time_s] * freq_std[start_time_s])
                     + freq_mod[start_time_s])
...
def apply_equalizer(signal, flight_state, sampling_rate, window_length, equalizer_settings_db):
    ...
    for i in range(Zxx.shape[1]):
        ...
        # change states
        if (i * sampling_rate * window_length / 2 > flight_state[state_ind][1]
                and state_ind < len(flight_state) - 1):
            state_ind += 1
            state = flight_state[state_ind][0]
            equalizer_settings_amplitude = 10 ** (equalizer_settings_db[state] / 20)
...
def getFullDroneSignal(simple_noise_flag, flight_state, rpm_matrix, rpm_std, freq_mod,
                       sampling_rate, drone_loudness, drone_type="M2", signaltype='pulse',
                       white_noise=0.01, num_rotor=4, change_time=0.1, s_shape_coef=5, pulse_len=1):
    full_drone_signal = np.zeros(int(rpm_matrix[-1][-1] * sampling_rate))
    rpm_freqs = []

    for i in range(num_rotor):
        random_number = random.random()
        one_rotor_signal, rpm_freq = getOneRotorSignal(
            rpm_matrix, rpm_std, freq_mod, sampling_rate, drone_loudness,
            signaltype, change_time, s_shape_coef, pulse_len, random_number)
    ...
    if drone_type != 'F-4':
        ...
        full_drone_signal = apply_equalizer(
            full_drone_signal, flight_state, sampling_rate, window_length, equalizer_settings_db)
