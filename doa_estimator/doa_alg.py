...
def calculate_direction_vector_from_time_differences(mic_pos_matrix, angle_deg1, angle_deg2, angle_deg3, size_m):
    v1 = mic_pos_matrix[0] - mic_pos_matrix[1]
    v2 = mic_pos_matrix[0] - mic_pos_matrix[2]
    v3 = mic_pos_matrix[0] - mic_pos_matrix[3]
    A = np.array([v1, v2, v3])
    b = size_m*np.array([np.cos(np.radians(angle_deg1)), np.cos(np.radians(angle_deg2)), np.cos(np.radians(angle_deg3))])
    x = np.linalg.solve(A, b)
    normalized_x = x / np.linalg.norm(x)
    return normalized_x


def get_direction_gcc_phat(signal, window_period, overlap_rate, mic_pos_matrix, sampling_rate, speed_of_sound, size_m, interp=512):
    ...
    for i in range(num_windows):
        ...
        tau1, _ = gcc_phat(window_signal0, window_signal1, sampling_rate, md1/speed_of_sound, interp)
        tau2, _ = gcc_phat(window_signal0, window_signal2, sampling_rate, md2/speed_of_sound, interp)
        tau3, _ = gcc_phat(window_signal0, window_signal3, sampling_rate, md3/speed_of_sound, interp) 
    ...
    drone_direction_est = [calculate_direction_vector_from_time_differences(mic_pos_matrix, angle1_degrees[i], angle2_degrees[i], angle3_degrees[i], size_m) for i in range(len(angle1_degrees))]
    return drone_direction_est


def srp_phat(microphone_positions, microphone_signals, sphere_points, sampling_rate, speed_of_sound, ind, beta_flag, mask_flag, device):
    ...
    for i in range(num_microphones):
        for j in range(i + 1, num_microphones):
            ...
            for k in range(sphere_points.shape[1]):
                ...
                steered_response = cc[int(delay * sampling_rate) if int(delay * sampling_rate)>=0 else int(delay * sampling_rate + num_samples)]
...
def plot_srp_points(accumulator, sphere_points, ind):
    ...
    plt.scatter(max_azimuth, max_elevation, color='blue', marker='x', s=50, label=f'Max Value: {max_value}\nAzimuth: {max_azimuth}\nElevation: {max_elevation}')
    plt.legend()
...
def get_doa_srp_phat(signal, frame_length, frame_hop, mic_pos_matrix, sampling_rate, fib_num, speed_of_sound, beta, mask, device):
    ...
    for i in range(frame_num):
        ...
        # srp_phat -- cart
        doa_est.append(srp_phat(mic_pos_matrix, window_signals, sphere_points, sampling_rate, speed_of_sound, i, beta, mask, device))
    return doa_est
